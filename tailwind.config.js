/** @type {import('tailwindcss').Config} */
module.exports = {
  content: [
    "./src/**/*.{html,ts}"
  ],
  safelist: [
    'text-orange-600',
    'bg-orange-600/25',
    'hover:bg-orange-600/50',
    'focus:bg-orange-600/50',
    'outline-orange-600',

    'text-orange-400',
    'bg-orange-400/25',
    'hover:bg-orange-400/50',
    'focus:bg-orange-400/50',
    'outline-orange-400',

    'text-rose-600',
    'bg-rose-600/25',
    'hover:bg-rose-600/50',
    'focus:bg-rose-600/50',
    'outline-rose-600',

    'text-sky-600',
    'bg-sky-600/25',
    'hover:bg-sky-600/50',
    'focus:bg-sky-600/50',
    'outline-sky-600',

    'text-yellow-400',
    'bg-yellow-400/25',
    'hover:bg-yellow-400/50',
    'focus:bg-yellow-400/50',
    'outline-yellow-400',

    'text-yellow-300',
    'bg-yellow-300/25',
    'hover:bg-yellow-300/50',
    'focus:bg-yellow-300/50',
    'outline-yellow-300',

    'text-cyan-500',
    'bg-cyan-500/25',
    'hover:bg-cyan-500/50',
    'focus:bg-cyan-500/50',
    'outline-cyan-500',

    'text-red-500',
    'bg-red-500/25',
    'hover:bg-red-500/50',
    'focus:bg-red-500/50',
    'outline-red-500',

    'text-red-700',
    'bg-red-700/25',
    'hover:bg-red-700/50',
    'focus:bg-red-700/50',
    'outline-red-700',

    'text-blue-500',
    'bg-blue-500/25',
    'hover:bg-blue-500/50',
    'focus:bg-blue-500/50',
    'outline-blue-500',

    'text-blue-400',
    'bg-blue-400/25',
    'hover:bg-blue-400/50',
    'focus:bg-blue-400/50',
    'outline-blue-400',

    'text-blue-700',
    'bg-blue-700/25',
    'hover:bg-blue-700/50',
    'focus:bg-blue-700/50',
    'outline-blue-700',

    'text-purple-600',
    'bg-purple-600/25',
    'hover:bg-purple-600/50',
    'focus:bg-purple-600/50',
    'outline-purple-600',

    'text-violet-500',
    'bg-violet-500/25',
    'hover:bg-violet-500/50',
    'focus:bg-violet-500/50',
    'outline-violet-500',

    'text-amber-300',
    'bg-amber-300/25',
    'hover:bg-amber-300/50',
    'focus:bg-amber-300/50',
    'outline-amber-300',

    'text-green-600',
    'bg-green-600/25',
    'hover:bg-green-600/50',
    'focus:bg-green-600/50',
    'outline-green-600',
  ],
  theme: {
    extend: {},
  },
  plugins: [],
}
